...
if ping -c 1 -W 1 "$NAS_HOST" > /dev/null 2>&1; then
  ...
  if "$NAS_SSH_BIN" -i "$NAS_SSH_KEY" ${(z)NAS_SSH_OPTS} "$NAS_SSH_TARGET" "sudo find /volume2/Media/Comics /volume2/Media/Films /volume2/Media/Music /volume2/Media/Series \
