...
if [[ "$MODE" == "list" ]]; then
  "$NAS_SSH_BIN" -i "$NAS_SSH_KEY" ${(z)NAS_SSH_OPTS} "$NAS_SSH_TARGET" "find /volume2/Media/Comics /volume2/Media/Films /volume2/Media/Series \
    -type f -name '.smbdelete*' -print -o -type d -empty -print"
else
  "$NAS_SSH_BIN" -i "$NAS_SSH_KEY" ${(z)NAS_SSH_OPTS} "$NAS_SSH_TARGET" "sudo find /volume2/Media/Comics /volume2/Media/Films /volume2/Media/Series \
